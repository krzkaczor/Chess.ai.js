...
describe('State Generator', function() {
  it('should generate all possible child states for game state', function() {
    ...
    if (blackStates[0].state.toFenNotation() == "8/8/p7/8/8/8/P7/8") {
      ...
      expect(blackStates[1].state.toFenNotation()).to.be.equal("8/8/p7/8/8/8/P7/8");
      expect(blackStates[1].action).to.be.eql({source: tu.makeMove(6, 0), target: tu.makeMove(5, 0)});
      expect(blackStates[0].state.toFenNotation()).to.be.equal("8/8/8/p7/8/8/P7/8");
      expect(blackStates[0].action).to.be.eql({source: tu.makeMove(6, 0), target: tu.makeMove(4, 0)});
    }
  });
});